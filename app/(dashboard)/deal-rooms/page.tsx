...
import { DealPipelineBoard, type PipelineDeal } from "./DealPipelineBoard";
...
export default async function DealRoomsPage({ searchParams }: PageProps) {
  const { tab } = await searchParams;
  const activeTab = tab === "pipeline" ? "pipeline" : "rooms";
  ...
  const [{ data, error }, { data: pipelineRaw }] = await Promise.all([
    supabase
      .from("deals")
    ...
    supabase
      .from("deals")
      .select(
        "id, deal_room_id, title, stage, updated_at, deal_rooms(assets(title), mandates(title))"
      )
      .eq("company_id", companyId)
      .order("updated_at", { ascending: false }),
  ]);
  ...
  const pipelineDeals: PipelineDeal[] = (pipelineRaw ?? []).map((d: any) => ({
    id: d.id,
    deal_room_id: d.deal_room_id,
    title: d.title,
    stage: d.stage,
    updated_at: d.updated_at,
    assetTitle: d.deal_rooms?.assets?.title ?? null,
    mandateTitle: d.deal_rooms?.mandates?.title ?? null,
  }));
  ...
  return (
    <div className="mx-auto max-w-6xl">
      ...
      <nav className="mt-6 flex gap-1 border-b border-gray-200" aria-label="Deal rooms views">
        <Link
          href="/deal-rooms"
      ...
      {activeTab === "pipeline" ? (
        <DealPipelineBoard deals={pipelineDeals} />
      ) : (
        <>
